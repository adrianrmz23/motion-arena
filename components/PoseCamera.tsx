...
export type MotionMove='LEFT JAB'|'RIGHT JAB'|'DODGE LEFT'|'DODGE RIGHT'|'CROUCH'|'GUARD';

type Props={
  onReady?:(ready:boolean)=>void;
  onMove?:(move:MotionMove)=>void;
...
export default function PoseCamera({onReady,onMove,onSnapshot,compact=false,gameMode=false,autoStart=false}:Props){
  ...
  const guardLatched=useRef(false);
  ...
  const fire=useCallback((name:MotionMove)=>{
    const now=performance.now();
    const gap=name==='GUARD'?900:430;
    if((cooldown.current[name]??0)+gap>now) return;
  ...
  const draw=(points:Point[])=>{
    ...
    ctx.lineWidth=gameMode?4:3; ctx.lineCap='round'; ctx.strokeStyle='rgba(93,240,255,.82)';
    CONNECTIONS.forEach(([a,b])=>{
      if((points[a]?.visibility??0)<.35||(points[b]?.visibility??0)<.35)return;
      const p=px(points[a]),q=px(points[b]);
      ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.stroke();
    });
    [0,11,12,13,14,15,16,23,24,25,26,27,28].forEach(i=>{
      const p=points[i];if(!p||(p.visibility??0)<.35)return;
      const q=px(p);ctx.beginPath();ctx.arc(q.x,q.y,i===15||i===16?8:5,0,Math.PI*2);
      ctx.fillStyle=i===15||i===16?'#ff6a3d':'#eaff55';ctx.fill();
    });
  };
    ...
    if(video.currentTime!==lastVideoTime.current){
      ...
      if(points.length){
        ...
        if(now-lastUiUpdate.current>80){
          setSnapshot(s); onSnapshot?.(s); lastUiUpdate.current=now;
        }
        if(s.leftJab) fire('LEFT JAB');
        if(s.rightJab) fire('RIGHT JAB');
        if(s.dodge) fire(`DODGE ${s.dodge.toUpperCase()}` as MotionMove);
        if(s.crouch) fire('CROUCH');
        if(s.guard&&!guardLatched.current){fire('GUARD');guardLatched.current=true;}
        if(!s.guard) guardLatched.current=false;
        prevRef.current=points.map(p=>({...p}));
      } else {
        if(performance.now()-lastUiUpdate.current>100){setSnapshot(empty);onSnapshot?.(empty);lastUiUpdate.current=performance.now();}
      }
    }
  ...
  const start=async()=>{
    ...
    try{
      ...
      const options={baseOptions:{modelAssetPath:'https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_lite/float16/1/pose_landmarker_lite.task'},runningMode:'VIDEO' as const,numPoses:1,minPoseDetectionConfidence:.5,minPosePresenceConfidence:.5,minTrackingConfidence:.5};
      try{
        landmarkerRef.current=await PoseLandmarker.createFromOptions(vision,{...options,baseOptions:{...options.baseOptions,delegate:'GPU'}});
      }catch{
  ...
  useEffect(()=>{if(autoStart&&status==='idle')void start();},[autoStart,status]);
  ...
  return <div className={`poseStation ${compact?'poseCompact':''} ${gameMode?'poseGameMode':''}`}>
    ...
    {!gameMode&&<div className="trackingPanel">
      <div className="trackingHeader"><div><span>MOTION ENGINE v0.3</span><h3>Calibración en vivo</h3></div><div className={`signal ${ready?'good':''}`}>{Math.round(snapshot.confidence*100)}%</div></div>
